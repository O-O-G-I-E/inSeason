<script>
	...
	function updateFilters() {
		...
		const base = resolve('/monat/[month]', { month: `${month}` });
		history.replaceState(history.state, '', base + (query ? `?${query}` : ''));
	}
	...
	function resetFilters() {
		history.replaceState(history.state, '', resolve('/monat/[month]', { month: `${month}` }));
	}
</script>
...
<div class="month-section">
	...
	<div class="months-grid">
		{#each monthOptions as monthOption (monthOption)}
			<a
				href={resolve('/monat/[month]', { month: `${monthOption}` })}
				class="month-btn"
				class:selected={monthOption === month}
				class:current={monthOption === currentMonth}
			>
				{getShortMonthName(monthOption)}
			</a>
		{/each}
	</div>
</div>
